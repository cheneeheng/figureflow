...
status: ready
created: 2026-06-03
scope: Serialization — lossless JSON save/load round-trip of a diagram, plus lossy mermaid flowchart export. Import (foreign grammar → figureflow) is explicitly deferred.
...
## §03 · Tech Stack
- **New module:** `src/figureflow/serialize.py` (declared in SKELETON § 04
...
per-element dicts the channel already uses (so it is lossless against figureflow's
own schema):
```
{ "schema": "figureflow/1",
...
> grammars *into* figureflow needs a foreign-grammar parser plus a layout pass for
