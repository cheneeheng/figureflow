...
status: ready
created: 2026-06-03
app: pyxyflow — interactive flowchart widget for Python
...
mvp_target: A pip-installable anywidget library (offline/self-contained front-end, no CDN) that renders an interactive flowchart in any Jupyter host, with per-element shape + styling for nodes and edges, drag edits synced back to Python, multi-select + within-canvas styled copy/paste, a minimap, undo/redo of canvas edits, node grouping, automatic layout, lossless JSON save/load plus mermaid export, and a custom JS/TS component escape hatch — publishable as v0.1.
---
...
pyxyflow lets a Python user build and display an interactive flowchart without
...
- **Front-end (bundled, offline).** Source ESM in `js/`, bundled by **esbuild**
  into `src/pyxyflow/static/widget.js` with **React 18.3.1** and
...
- **Typings**: hand-written `.d.ts` shipped alongside the bundle so L3
  ...
  artifact pyxyflow loads at runtime is plain JS.

## §04 · Python layer (`src/pyxyflow/__init__.py`)
...
src/pyxyflow/
  ...
  static/
    ...
    pyxyflow.d.ts        # custom-component typings (added ITER_06)
```
...
- *Maintainer:* `npm install && npm run build` (esbuild → `static/widget.js`),
  then `pip install -e .`.
- *Notebook:* `from pyxyflow import Flow, Node; Flow(nodes=[Node("a","Hi")])`.
...
- **Format *import*** (parsing mermaid or other grammars *into* pyxyflow).
